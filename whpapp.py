...
st.header("Well Data")
# Load the model
model_rate = '/Users/rianrachmanto/pypro/project/gas-well-mon/model/model_rate.pkl/model.pkl'
model_whp='/Users/rianrachmanto/pypro/project/gas-well-mon/model_whp/model.pkl/model.pkl'
with open(model_rate, 'rb') as file:
    model_rate = pickle.load(file)
with open(model_whp, 'rb') as file:
    model_whp = pickle.load(file)
...
def main():
    ...
    # Get the latest data from df_well
    ...
    # Slider for choke size adjustment
    choke_size = st.slider('Choke Size', min_value=2, max_value=256, value=int(latest_data['CHK']), step=2)

    # Prepare input for WHP prediction
    whp_input = [latest_data['CSG'], latest_data['SEP'], choke_size]
    # Predict WHP based on the new choke size
    predicted_whp = model_whp.predict([whp_input])[0]

    # Prepare input for GAS rate prediction
    gas_rate_input = [latest_data['CSG'], latest_data['SEP'], latest_data['GOR'], latest_data['OIL'], choke_size, predicted_whp]
    # Predict new GAS rate with the new WHP and CHK
    predicted_gas_rate = model_rate.predict([gas_rate_input])[0]
    
    # Display the results
    st.write("Predicted WHP with updated Choke Size:", f"{predicted_whp:.2f} PSI")
    st.write("Predicted Gas Rate with updated Choke Size:", f"{predicted_gas_rate:.2f} MMSCFD")
