...
def process_data(X_train,X_test):
    #select features
    X_train=X_train[['WHP','CSG','SEP','GOR','OIL','CHK']]
    X_test=X_test[['WHP','CSG','SEP','GOR','OIL','CHK']]
...
def main():
    ...
    #make plot between CHK vs actual and predicted on the same plot
    # plt.figure(figsize=(10,6))
    # plt.scatter(X_test['CHK'],y_test, alpha=0.3, label='Actual')
    # plt.scatter(X_test['CHK'],y_pred, alpha=0.3, label='Predicted')
    # plt.xlabel('CHK')
    # plt.ylabel('GAS')
    # plt.title('CHK vs GAS')
    # plt.legend()
    # plt.show()
