...
st.header("Well Data")
# Load the model
model_path = 'P:/project/pythonpro/myvenv/gas-well-mon/model/model.pkl'
...
def main():
    ...
    df_pred = {
        ...
        'CSG': latest_data['CSG'],
        'SEP': latest_data['SEP'],
        'BS&W': latest_data['BS&W'],
        'GOR': latest_data['GOR'],
        'WTR': latest_data['WTR'],
        'OIL': latest_data['OIL']
    }

    # Input new gas rate and predict
    st.subheader('Input Desired Gas Rate')
    gas = st.text_input("Put your new value of gas here")
    if st.button("Predict New Choke Size"):
        if not gas:
            st.error("Please enter a gas rate value")
        else:
            try:
                gas_value = float(gas)
                if gas_value <= 0:
                    st.error("Gas rate must be greater than 0")
                else:
                    df_pred['GAS'] = gas_value
                    features = pd.DataFrame([df_pred])
                    pred = model.predict(features)
                    st.subheader(f"Predicted Choke Size: {pred[0]:.2f}")
            except ValueError:
                st.error("Please enter a valid number for gas rate")
